...
posmo_trips |>
  group_by(segment_id) |>
  ggplot() +
  geom_boxplot(aes(day_week, trip_duration/60)) +
  labs(title = "mean trip duration compared between weekend/weekday")
...
Lineplot with hours an weekdays, number of trips
```{r}
posmo_hours |>
  group_by(segment_id) |>
  ggplot() +
  geom_line(aes(time_round, colour = weekday)) +
  labs(title = "mean trip distance per hour of the day")
```
