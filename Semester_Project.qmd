...
### remove outliers 
Ich habe in den Daten gesehen, dass sogar der Transport mode Airplane drin ist. Diesen würden wir sowieso rausnehmen. Ich denke die Begrenzung anhand der schweizer grenze ist aber schon ok sonst :).
```{r}
...
# join data together ---------------------------------------------
weather <- left_join(legend, weather, by = "stn")

# create sf object --------------------------------------------------
# empty units are created???? 
weather <- st_as_sf(weather, coords = c("E","N"), crs = 4326) |>
  st_transform(2056)
