...
## OK - Data consistency overview and removal of inconsistent datasets
```{r}
...
# save cleaned dataset in an .gpkg file
st_write(posmo_cut, "posmo_data/posmo_cut2.gpkg")
```
...
posmo <- read_sf("posmo_data/posmo_cut2.gpkg")
...
st_write(posmo_added, "posmo_data/posmo_end1.gpkg")

# save clean data in .csv 
write_csv(posmo_sep, "posmo_data/posmo_end1.csv")
```
...
# visualize 
ggplot() +
  geom_boxplot(data = mean_travldis_per_date, aes(dataset, travldis_day)) +
...
# visualize 
ggplot() +
  geom_boxplot(data = mean_travldis_per_date_week, aes(day_week, travldis_day)) +
...
```{r}
#Steplength rain Influence -------------------------------------
#But we want mean of travelling distance ...  over all travel modes 
mean_travldis_per_rain <- posmo |> 
  mutate(steplength_m = sqrt((X - lead(X))^2 + (Y - lead(Y))^2))|>  
...
# visualize 
ggplot() +
  geom_boxplot(data = mean_travldis_per_rain, aes(rain, travldis_day)) +
...
#But we want mean of travelling distance ...  for BIKE  
mean_travldis_per_rain_bike <- posmo_bike |> 
  mutate(steplength_m = sqrt((X - lead(X))^2 + (Y - lead(Y))^2))|>  
  group_by(rain, date, dataset) |> 
  summarise(travldis_day = mean(steplength_m, na.rm = T))
...
# visualize 
ggplot() +
  geom_boxplot(data = mean_travldis_per_rain_bike, aes(rain, travldis_day)) +
  labs(title = "mean travelled distance compared between rain/no rain for bike") 
```
...
day_hour <- function(dataset = posmo, day, number){ 
  day_stat <- dataset %>% 
    filter(weekday == day) %>% 
    group_by(start_hour) %>% 
    summarize(count = n()) %>% 
