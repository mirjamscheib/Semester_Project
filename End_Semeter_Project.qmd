...
et al., 2020). Nevertheless, most studies do not focus on specific
social groups (e.g. workforce, students, pensioners), that might follow
distinct spatio-temporal patterns, showing a potential demand for a
specific adaption of infrastructure. The canton of Zurich accommodates
the most students in Switzerland, with numbers continuing to rise
(Bundesamt für Statistik, 2023; Medienmitteilung Kanton ZH, 2021).
Additionally, students and apprentices (from the age of 15 years) make
up 39% of the public transport commuter mass, which is why knowledge of
spatio-temporal patterns of students could help to manage train and
other public transport infrastructure (Bundesamt für Statistik, 2021).
Therefore, we want to investigate the influence of three environmental
factors on spatio-temporal movement patterns of students, aiming to
answer the following research questions:

Does the day of the week (weekend vs. workday) have an influence on the
spatio-temporal movement patterns of students?
...
Does the weather condition (e.g. precipitation) have an impact the
spatio-temporal patterns of students?
...
ggplot(posmo_day, aes(hour, mean_dis, col = day_week)) +
  geom_point() +
  geom_line(lwd = 0.7) +
  labs(title = "Mean Trip Distance over 24h", subtitle = "Compared between Weekend vs. Weekday", color = "weekend / weekday") +
...
ggplot() +
  geom_boxplot(data = posmo, aes(transport_mode, log(trip_dis), fill = rain_day)) +
    labs(title = "Trip distances per Transport Mode", subtitle = "Comparing rainy with dry days", fill = "precipitation") +
  ylab("Log Distance [m]") +
  xlab("Transport Mode") +
...
ggplot(posmo, aes(transport_mode, log(trip_speed), fill = rain_day)) +
  geom_boxplot() +
   labs(title = "Speed per Transport Mode", subtitle = "Comparing rainy with dry days", fill = "precipitation") +
  ylab("Log Speed [m/s]") +
  xlab("Transport Mode") +
  scale_fill_manual(values = c("no_rain" = "lavenderblush3", "rain" = "aquamarine3"), labels = c( "No", "Yes")) +
  theme_minimal() 
...
ggplot() +
  ...
  labs(title = "Trip duration per Transport Mode", subtitle = "Comparing rainy with dry days", fill = "precipitation") +
  ylab("Duration [min]") +
  xlab("Transport Mode") +
  scale_fill_manual(values = c("no_rain" = "lavenderblush3", "rain" = "aquamarine3"), labels = c( "No", "Yes")) +
  theme_minimal()
