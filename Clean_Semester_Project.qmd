...
```{r}
# load clean data 
posmo <- read_delim("posmo_data/posmo_clean2.csv")
```
...
ggplot(posmo, aes(transport_mode, log(steplength), fill = day_week)) +
  ...
  xlab("Transport Mode") +
  scale_fill_manual(values = c("weekday" = "red", "weekend" = "blue"), labels = c( "Weekday", "Weekend")) +
  theme_minimal()
...
posmo_trips <- read_delim("posmo_data/posmo_trips.csv")
```

#### Farben anpassen - Duration 
...
#mean trip distance compared between weekend/weekday seems OK
  ggplot() +
  geom_boxplot(data = posmo_trips, aes(transport_mode, log(trip_dis), fill = day_week)) +
...
tm_shape(mcp_posmo_trips) +
  tm_fill(col = "dataset", alpha = 0.4) +
  tm_legend(title = "User") +
  tm_shape(mcp_posmo_trips) +
  tm_borders(col = "red") +
  tm_shape(posmo_trips) +
  tm_dots(col = "day_week") +
  tm_legend(title = "Day of the Week", labels = c("Weekend", "Weekday"))
```
...
#### Legende anschreiben - Analysis with unsegmented posmo data
```{r}
...
ggplot(posmo, aes(transport_mode, log(steplength), fill = rain_day)) +
  geom_boxplot() +
  labs(title = "Travelled Distance per Transport Mode comparing Rainy Days vs. Not Rainy Days")

# Boxplot compare weekend/ weekday without grouping (overall speed)
ggplot(posmo, aes(transport_mode, log(speed), fill = rain_day)) +
  geom_boxplot() +
  labs(title = "Speed per Transport Mode comparing Rainy Days vs. Not Rainy Days")
...
```{r}
# subset travel_modes for more closer look 
mean_rain <- posmo |> 
...
ggplot() +
  ...
  labs(title = "Travelled distance compared between rainy days vs. not rainy days") 
...
ggplot() +
  ...
  labs(title = "Velocity compared between rainy days vs. not rainy days") 
```
...
posmo_trips |>
  group_by(segment_id) |>
  ggplot() +
  geom_boxplot(aes(transport_mode, trip_duration/60, fill = rain_day)) +
  labs(title = "mean trip duration [min] compared between rainy days vs. not rainy days")

# mean trip distance compared between rain/no rain
posmo_trips |>
  group_by(segment_id) |>
  ggplot() +
  geom_boxplot(aes(transport_mode, trip_dis, fill = rain_day)) +
  labs(title = "mean trip distance compared between rainy days vs. not rainy days")
```
