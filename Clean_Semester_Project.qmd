...
ggplot(posmo, aes(transport_mode, log(steplength), fill = day_week)) +
  ...
  xlab("Transport Mode") +
  scale_color_manual(values = c("weekday" = "red", "weekend" = "blue"), labels = c( "Weekday", "Weekend")) +
  theme_minimal()
...
posmo_trips |>
  group_by(segment_id) |>
  ggplot() +
  geom_boxplot(aes(transport_mode, trip_duration/60, fill = day_week)) +
...
posmo_trips |>
  group_by(segment_id) |>
  ggplot() +
  geom_boxplot(aes(transport_mode, trip_dis, fill = day_week)) +
...
#### Analysis with unsegmented posmo data
```{r}
...
#### Analysis with segmented posmo data 
```{r}
